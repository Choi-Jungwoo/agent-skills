# CTP trader SPI order and trade notifications

This reference covers asynchronous notifications for orders, quotes, trades, exec orders, option self-close, for-quote, and offset settings.

| Entry | Kind |
| --- | --- |
| OnRtnCombAction | callback |
| OnRtnErrorConditionalOrder | callback |
| OnRtnExecOrder | callback |
| OnRtnForQuoteRsp | callback |
| OnRtnOptionSelfClose | callback |
| OnRtnOrder | callback |
| OnRtnQuote | callback |
| OnRtnTrade | callback |
| OnRtnOffsetSetting | callback |

## OnRtnCombAction

`OnRtnCombAction` is an event callback that reports state back from the CTP runtime into the client application.
...
`OnRtnErrorConditionalOrder` is an event callback that reports state back from the CTP runtime into the client application.
...
**Field guide for `CThostFtdcErrorConditionalOrderField`**
...
```cpp
void MySpi::OnRtnErrorConditionalOrder() {
    // Mark the event in your session state machine and send the next step if needed.
}
```
...
`OnRtnExecOrder` is an event callback that reports state back from the CTP runtime into the client application.
...
`OnRtnOptionSelfClose` is an event callback that reports state back from the CTP runtime into the client application.
...
```cpp
void MySpi::OnRtnOptionSelfClose() {
    // Mark the event in your session state machine and send the next step if needed.
}
```
...
`OnRtnOrder` is an event callback that reports state back from the CTP runtime into the client application.

- Kind: callback
- Owning interface: `CThostFtdcTraderSpi`
- Related symbols: `ReqOrderInsert`, `OnRtnTrade`
...
`OnRtnQuote` is an event callback that reports state back from the CTP runtime into the client application.
...
`OnRtnTrade` is an event callback that reports state back from the CTP runtime into the client application.
...
`OnRtnOffsetSetting` is an event callback that reports state back from the CTP runtime into the client application.

- Kind: callback
- Owning interface: `CThostFtdcTraderSpi`
- Related symbols: `ReqOffsetSetting`, `ReqCancelOffsetSetting`
